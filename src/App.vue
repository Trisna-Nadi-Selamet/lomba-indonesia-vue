...
// function getEvent(id) { // return events.value.find((e) => e.id === id); // } // function eventDateTime(event) { // return new Date(`${event.date}T${event.startTime}:00`); // } // function countdown(event) { // const diff =
eventDateTime(event).getTime() - now.value.getTime(); // if (diff <= 0) return { expired: true, days: 0, hours: 0, minutes: 0, seconds: 0 }; // const s = Math.floor(diff / 1000); // return { expired: false, days: Math.floor(s / 86400),
hours: Math.floor((s % 86400) / 3600), minutes: Math.floor((s % 3600) / 60), seconds: s % 60 }; // } // function countdownText(event) { // const c = countdown(event); // if (c.expired) return 'DIMULAI'; // return `${c.days}h
${String(c.hours).padStart(2, '0')}j ${String(c.minutes).padStart(2, '0')}m ${String(c.seconds).padStart(2, '0')}d`; // } // function formatDate(date) { // return new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium' }).format(new
Date(date + 'T00:00:00')); // }
...
const eventForm = ref({
  ...
  address: 'Jl. Kemerdekaan No. 17, Kelurahan Setempat',
  quota: 20,
  status: 'open',
});
...
const paginatedEvents = computed(() => {
  ...
  const end = start + eventPageSize;

  return events.value.slice(start, end);
});

const eventTotalPages = computed(() => {
  return Math.max(1, Math.ceil(events.value.length / eventPageSize));
});
...
async function loadPublicData() {
  ...
  try {
    ...
    /*
     * Jika jumlah event berkurang setelah delete,
     * pastikan halaman event tidak melebihi halaman terakhir.
     */
...
async function loadParticipants() {
  ...
  try {
    ...
    let query = supabase
      .from('participants')
      .select('id,name,phone,group_name,age,address,event_id,registered_at,events(name)', {
        count: 'exact',
      })
      .order('registered_at', {
        ascending: false,
    ...
    if (q) {
      ...
      if (eventMatches.length) {
        clauses.push(`event_id.in.(${eventMatches.join(',')})`);
      }
...
function openRegistration(event = null) {
  registration.value = {
    ...
    age: '',
    address: '',
    eventId: event?.id || events.value[0]?.id || '',
  };
...
async function submitRegistration() {
  ...
  if (!r.name.trim() || !r.phone.trim() || !r.eventId) {
    return notify('Nama, nomor HP, dan lomba wajib diisi.');
  }
...
async function saveEvent() {
  ...
  if (!f.name || !f.date || !f.startTime || !f.location) {
    return notify('Lengkapi data perlombaan.');
  }
...
async function deleteEvent(event) {
  ...
  if (!confirm(`Hapus lomba "${event.name}"? Peserta lomba ini juga akan terhapus.`)) {
    return;
  }
...
async function importJSON(event) {
  ...
  try {
    ...
    if (!Array.isArray(parsed.events) || !Array.isArray(parsed.participants)) {
    ...
    const orgRows = (parsed.organizers || []).map((o) => ({
      id: o.id,
      name: o.name,
      role: o.role,
      phone: o.phone || null,
    ...
    const memRows = (parsed.members || []).map((m) => ({
      id: m.id,
      name: m.name,
      group_name: m.group_name || m.group || 'Umum',
      phone: m.phone || null,
    }));
...
function goTo(tab) {
  ...
  if (tab === 'participants' && isAdmin.value) {
    loadParticipants();
  }
  ...
  <div class="app">
    ...
    <header class="hero">
      <nav class="nav container mobile-nav">
        ...
        <!-- HAMBURGER -->
        <button type="button" class="hamburger-btn" :class="{ open: mobileMenuOpen }" @click="mobileMenuOpen = !mobileMenuOpen" aria-label="Buka menu" :aria-expanded="mobileMenuOpen">
          <span></span>
          <span></span>
          <span></span>
        </button>
        ...
        <Transition name="menu-slide">
          <div v-if="mobileMenuOpen" class="mobile-menu">
            <button :class="{ active: activeTab === 'home' }" @click="goTo('home')">
              <Home :size="17" />
              Home
            </button>

            <button :class="{ active: activeTab === 'events' }" @click="goTo('events')">
              <Trophy :size="17" />
              Lomba
            </button>

            <button :class="{ active: activeTab === 'participants' }" @click="goTo('participants')">
              <Users :size="17" />
              Peserta
            </button>

            <button :class="{ active: activeTab === 'organizers' }" @click="goTo('organizers')">
              <ShieldCheck :size="17" />
              Pengurus
            </button>

            <button :class="{ active: activeTab === 'members' }" @click="goTo('members')">
              <UserRound :size="17" />
              Anggota
            </button>
      ...
      <section class="hero-content container">
        <div class="hero-copy">
          ...
          <div class="hero-meta">
            <span>
              <MapPin :size="14" />
              Lokasi & alamat lengkap tersedia di setiap lomba
            </span>
          </div>

          <button class="primary" @click="openRegistration()">
            <CheckCircle2 :size="17" />
            Daftar Sekarang
          </button>
          ...
          <button class="primary" @click="downloadDocument()">
            <Download :size="17" />
            Proposal Kami
          </button>
        </div>
    ...
    <main class="container main">
      ...
      <div v-if="errorMessage" class="connection-warning">
        ...
        <span>
          {{ errorMessage }}
          Pastikan URL dan key Supabase sudah diatur.
        </span>

        <button class="outline" @click="loadPublicData">
          <RefreshCw :size="14" />
          Coba Lagi
        </button>
      </div>
      ...
      <section v-if="activeTab === 'home'" class="tab-fade">
        ...
        <div v-else class="event-grid">
          <article v-for="event in paginatedEvents" :key="event.id" class="event-card">
            ...
            <button class="primary full" @click="openRegistration(event)" :disabled="event.status !== 'open' || (eventCounts[event.id] || 0) >= event.quota">
              {{ event.status === 'open' ? 'Daftar' : 'Ditutup' }}
            </button>
          </article>
        </div>
        ...
        <!-- PAGINATION LOMBA -->
        <div
  v-if="eventTotalPages > 1"
  class="pagination event-pagination"
>
  <button
    class="page-btn"
  ...
  >
    ›
      ...
      <section v-if="activeTab === 'events'" class="tab-fade">
        <div class="section-head">
          ...
          <button class="primary" @click="openEventForm()">
            <Plus :size="17" />
            Tambah Lomba
          </button>
        </div>
        ...
        <div class="event-grid">
          <article v-for="event in paginatedEvents" :key="event.id" class="event-card">
            ...
            <div class="card-actions">
              <button v-if="isAdmin" class="outline" @click="openEventForm(event)">
                <Edit3 :size="15" />
                Edit
              </button>

              <button v-if="isAdmin" class="danger" @click="deleteEvent(event)">
                <Trash2 :size="15" />
                Hapus
              </button>
        ...
        <div v-if="eventTotalPages > 1" class="pagination event-pagination">
          ...
          <button class="page-btn" :disabled="eventPage === eventTotalPages" @click="changeEventPage(eventPage + 1)">›</button>
        </div>
      </section>
        ...
        <template v-else>
          <div class="section-head">
            ...
            <div class="head-actions">
              <button class="outline" @click="logout">
                <LogOut :size="15" />
                Logout
              </button>
          ...
          <div class="toolbar">
            <div class="search">
              ...
              <input v-model="search" @input="resetPage" placeholder="Cari nama, HP, kelompok, atau lomba..." />
            </div>

            <div class="data-actions">
              <button class="outline" @click="exportJSON">
                <Download :size="16" />
                Export JSON
              </button>

              <label class="outline file-btn">
                <Upload :size="16" />
                Import JSON
            ...
            <table>
              ...
              <tbody>
                ...
                <tr v-for="(p, i) in participantRows" :key="p.id">
                  <td>
                    {{ (participantPage - 1) * pageSize + i + 1 }}
                  </td>
                  ...
                  <td>
                    {{ p.events?.name || getEvent(p.eventId)?.name || '-' }}
                  </td>
                  ...
                  <td>
                    <button class="icon-btn danger-text" @click="deleteParticipant(p)">
                      <Trash2 :size="16" />
                    </button>
                  </td>
                </tr>
          ...
          <div class="pagination">
            <span>
              {{ totalParticipants }}
              peserta • 20 data per halaman
            </span>

            <div>
              <button class="icon-btn" :disabled="participantPage === 1 || participantLoading" @click="previousPage">
                <ChevronLeft />
              </button>
              ...
              <button class="icon-btn" :disabled="participantPage === totalPages || participantLoading" @click="nextPage">
                <ChevronRight />
              </button>
            </div>
          </div>
    ...
    <footer>
      <div class="container footer-main">
        <div>
          <b> MERDEKA INDONESIA </b>

          <span> Sistem Pendaftaran Lomba 17 Agustus 2026, Kp. Anggrek Tegalwaru Rt 02 Rw 05 Desa Tegalwaru Ciampea Bogor </span>
        </div>
    ...
    <Transition name="modal-fade">
      <div v-if="showRegistration" class="modal-backdrop" @click.self="showRegistration = false">
        <form class="modal" @submit.prevent="submitRegistration">
          <button type="button" class="close" @click="showRegistration = false">
            <X />
          </button>

          <span class="eyebrow"> FORM PENDAFTARAN </span>
          ...
          <label>
            ...
            <input v-model="registration.name" required maxlength="80" placeholder="Nama peserta" />
          </label>

          <label>
            No. HP

            <input v-model="registration.phone" required maxlength="20" inputmode="tel" placeholder="08xxxxxxxxxx" />
          </label>
          ...
          <div class="two">
            <label>
              ...
              <input v-model="registration.group" maxlength="50" placeholder="RT 02 / RW 05" />
            </label>

            <label>
              Usia

              <input v-model="registration.age" type="number" min="1" max="120" />
            </label>
          </div>
          ...
          <label>
            ...
            <select v-model="registration.eventId" required>
              <option v-for="e in events" :value="e.id" :key="e.id" :disabled="e.status !== 'open' || (eventCounts[e.id] || 0) >= e.quota">
                {{ e.name }}
                —
                {{ formatDate(e.date) }}

                {{ e.status !== 'open' ? '(Ditutup)' : '' }}
              </option>
            </select>
          </label>
          ...
          <label>
            ...
            <textarea v-model="registration.address" maxlength="200" rows="2"></textarea>
          </label>

          <button class="primary full" type="submit" :disabled="authLoading">
            <CheckCircle2 :size="17" />

            {{ authLoading ? 'Menyimpan...' : 'Simpan Pendaftaran' }}
          </button>
        </form>
      </div>
    </Transition>
    ...
    <Transition name="modal-fade">
      <div v-if="showEventForm" class="modal-backdrop" @click.self="showEventForm = false">
        <form class="modal" @submit.prevent="saveEvent">
          <button type="button" class="close" @click="showEventForm = false">
            <X />
          </button>

          <span class="eyebrow"> MANAGEMENT LOMBA </span>

          <h2>
            {{ editingEvent ? 'Edit' : 'Tambah' }}
            Perlombaan
          </h2>
          ...
          <label>
            ...
            <input v-model="eventForm.name" required maxlength="80" />
          </label>
          ...
          <div class="two">
            ...
            <label>
              ...
              <input v-model="eventForm.quota" type="number" min="1" max="10000" />
            </label>
          </div>
          ...
          <label>
            ...
            <input v-model="eventForm.location" required maxlength="100" placeholder="Lapangan Utama" />
          </label>

          <label>
            Alamat Lengkap Perlombaan

            <input v-model="eventForm.address" required maxlength="180" placeholder="Jl. Kemerdekaan No. 17, Kelurahan..." />
          </label>
    ...
    <Transition name="modal-fade">
      <div v-if="showLogin" class="modal-backdrop" @click.self="showLogin = false">
        <form class="modal login-modal" @submit.prevent="login">
          ...
          <span class="eyebrow"> PANITIA </span>
          ...
          <label>
            ...
            <input v-model="loginForm.email" type="email" autocomplete="email" required placeholder="admin@email.com" />
          </label>
